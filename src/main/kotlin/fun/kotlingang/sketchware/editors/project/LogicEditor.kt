...
class LogicEditor(
    ...
    companion object {
        suspend operator fun invoke(file: File) =
            LogicEditor(file.readOrNull()?.decrypt()?.bytesToString() ?: "", file)
    }
    ...
    fun editEventInfo(
        ...
        val events = getEvents(activity)?.toMutableList() ?: throw EventsNotFoundException(
            activity
        )
        val index = events.indexOfFirst { it.targetId == targetId && it.name == eventName }
            .takeIf { it != -1 } ?: throw EventNotFoundException(
            activity,
            targetId,
            eventName
        )
    ...
    fun editMoreblockInfo(
        ...
        val index = moreblocks.indexOfFirst { it.name == moreblock.name }
            .takeUnless { it == -1 } ?: throw MoreblockNotFoundException(
            activity,
            moreblock.name
        )
